...
│  ├─ Categorization            │  ├─ Context-aware responses     │
│  ├─ Recurring expenses        │  ├─ Personalized advice         │
│  └─ Receipt scanning (AI)     │  └─ Financial goal planning     │
├───────────────────────────────┼─────────────────────────────────┤
│  💰 BUDGET MANAGEMENT         │  📈 ANALYTICS & INSIGHTS        │
│  ├─ Category budgets          │  ├─ Spending patterns           │
│  ├─ Monthly limits            │  ├─ Trend analysis              │
│  ├─ Alert notifications       │  ├─ Predictive forecasting      │
│  └─ Progress tracking         │  └─ Category breakdown          │
├─────��─────────────────────────┼─────────────────────────────────┤
│  🔐 SECURITY                  │  ⚡ PERFORMANCE                  │
│  ├─ JWT Authentication        │  ├─ Redis caching               │
│  ├─ Role-based access         │  ├─ Optimized queries           │
│  ├─ API rate limiting         │  ├─ Async processing            │
│  └─ Data encryption           │  └─ Connection pooling          │
└─────────────────────────────────────────────────────────────────┘
```
...
│  │                      REST CONTROLLERS                               │    │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐  │    │
│  │  │   Auth   │ │ Transac- │ │  Budget  │ │ Analytics│ │    AI    │  │    │
│  │  │Controller│ │   tion   │ │Controller│ │Controller│ │Controller│  │    │
│  │  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘  │    │
│  └───────┼────────────┼────────────┼────────────┼────────────┼────────┘    │
│          │            │            │            │            │              │
│  ┌───────▼────────────▼────────────▼────────────▼────────────▼────────┐    │
│  │                      BUSINESS SERVICES                              │    │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐  │    │
│  │  │   User   │ │Transaction│ │  Budget  │ │ Analytics│ │    AI    │  │    │
...
│   ┌───────────────────┐              ┌───────────────────┐                 │
│   │                   │              │                   │                 │
│   │   AWS DYNAMODB    │              │   REDIS CACHE     │                 │
│   │   (Primary DB)    │◄────────────►│   (Lettuce)       │                 │
│   │                   │   Cache      │                   │                 │
│   │  ┌─────────────┐  │   Aside      │  ┌─────────────┐  │                 │
│   │  │ Users Table │  │   Pattern    │  │   Session   │  │                 │
│   │  ├─────────────┤  │              │  │    Cache    │  │                 │
│   │  │Transactions │  │              │  ├─────────────┤  │                 │
│   │  │   Table     │  │              │  │    Data     │  │                 │
│   │  ├─────────────┤  │              │  │    Cache    │  │                 │
...
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐            │    │
│  │  │SecurityConfig│ │ RedisConfig  │ │DynamoDBConfig│            │    │
│  │  └──────────────┘ └──────────────┘ └──────────────┘            │    │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐            │    │
│  │  │  AIConfig    │ │  WebConfig   │ │ AsyncConfig  │            │    │
│  │  └──────────────┘ └──────────────┘ └──────────────┘            │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                      SECURITY MODULE                             │    │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐            │    │
│  │  │ JwtProvider  │ │ JwtFilter    │ │AuthEntryPoint│            │    │
│  │  └──────────────┘ └──────────────┘ └──────────────┘            │    │
│  │  ┌──────────────┐ ┌──────────────┐                             │    │
│  │  │UserDetails   │ │PasswordEnc  │                             │    │
│  │  │Service       │ │   oder      │                             │    │
│  │  └──────────────┘ └──────────────┘                             │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
...
│  │                                                                  │    │
│  │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐  │    │
│  │  │  USER MODULE    │  │TRANSACTION MOD. │  │  BUDGET MODULE  │  │    │
│  │  │ ┌─────────────┐ │  │ ┌─────────────┐ │  │ ┌─────────────┐ │  │    │
...
│  │                                                                  │    │
│  │  ┌─────────────────┐  ┌─────────────────┐                       │    │
│  │  │ ANALYTICS MOD.  │  │   AI MODULE     │                       │    │
│  │  │ ┌─────────────┐ │  │ ┌─────────────┐ │                       │    │
│  │  │ │ Controller  │ │  │ │ Controller  │ │                       │    │
│  │  │ ├─────────────┤ │  │ ├─────────────┤ │                       │    │
│  │  │ │  Service    │ │  │ │  Service    │ │                       │    │
│  │  │ ├─────────────┤ │  │ ├─────────────┤ │                       │    │
│  │  │ │Aggregations │ │  │ │ AITools     │ │                       │    │
│  │  │ └─────────────┘ │  │ ├─────────────┤ │                       │    │
│  │  └─────────────────┘  │ │PromptTemplt │ │                       │    │
│  │                       │ ├─────────────┤ │                       │    │
│  │                       │ │ContextMgr   │ │                       │    │
...
│  │                      CACHE MODULE                                │    │
│  │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐            │    │
│  │  │CacheService  │ │CacheManager  │ │RateLimiter   │            │    │
│  │  └──────────────┘ └──────────────┘ └──────────────┘            │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
...
```
┌──────┐     ┌──────────┐     ┌───────────┐     ┌───────┐     ┌─────────┐     ┌────────┐
│Client│     │Controller│     │ AIService │     │ Redis │     │DynamoDB │     │ Gemini │
└──┬───┘     └────��─────┘     └─────┬─────┘     └───┬───┘     └────┬────┘     └───┬────┘
   │              │                 │               │              │              │
   │ POST /ai/chat│                 │               │              │              │
   │─────────────►│                 │               │              │              │
...
│  ║  AI Integration  │ Spring AI 1.0.x + Google GenAI                 ║  │
│  ╚═══════════════════════════════════════════════════════════════════╝  │
│                                                                          │
│  ╔═══════════════════════════════════════════════════════════════════╗  │
│  ║                        DATABASE                                    ║  │
│  ╠═══════════════════════════════════════════════════════════════════╣  │
│  ║  Primary DB      │ AWS DynamoDB (Free Tier)                       ║  │
│  ║  Cache           │ Redis 7.x with Lettuce Client                  ║  │
│  ║  Local Dev       │ DynamoDB Local + Redis Docker                  ║  │
│  ╚═══════════════════════════════════════════════════════════════════╝  │
│                                                                          │
│  ╔═══════════════════════════════════════════════════════════════════╗  │
│  ║                        FRONTEND                                    ║  │
│  ╠═══════════════════════════════════════════════════════════════════╣  │
│  ║  Option A        │ Thymeleaf + HTMX + TailwindCSS                 ║  │
│  ║  Option B        │ React 18 + Vite + TailwindCSS                  ║  │
...
│  MODULE │  REQUIREMENTS                                                     │
├─────────┼──────────────────────────────────────────────────────────────────┤
│         │  FR-1.1: User registration with email validation                 │
│  AUTH   │  FR-1.2: User login with JWT token generation                    │
│         │  FR-1.3: Token refresh mechanism                                 │
│         │  FR-1.4: Password reset functionality                            │
│         │  FR-1.5: Account deactivation                                    │
├─────────┼──────────────────────────────────────────────────────────────────┤
│         │  FR-2.1: Create income/expense transactions                      │
│  TRANS- │  FR-2.2: Update existing transactions                            │
│  ACTION │  FR-2.3: Delete transactions                                     │
│         │  FR-2.4: List transactions with filters (date, category, type)   │
│         │  FR-2.5: Search transactions by description                      │
│         │  FR-2.6: Bulk import transactions (CSV)                          │
│         │  FR-2.7: Recurring transaction support                           │
├─────────┼──────────────────────────────────────────────────────────────────┤
│         │  FR-3.1: Create category-based budgets                           │
│  BUDGET │  FR-3.2: Set monthly spending limits                             │
│         │  FR-3.3: Track budget utilization                                │
│         │  FR-3.4: Budget alerts at thresholds (50%, 75%, 90%, 100%)       │
│         │  FR-3.5: Budget history and comparisons                          │
├─────────┼──────────────────────────────────────────────────────────────────┤
│         │  FR-4.1: Spending breakdown by category                          │
│  ANALY- │  FR-4.2: Monthly/yearly trends                                   │
│  TICS   │  FR-4.3: Income vs expense comparison                            │
│         │  FR-4.4: Category-wise analysis                                  │
│         │  FR-4.5: Export reports (PDF/CSV)                                │
├─────────┼──────────────────────────────────────────────────��───────────────┤
│         │  FR-5.1: Natural language chat interface                         │
│         │  FR-5.2: Context-aware conversations (remembers history)         │
│   AI    │  FR-5.3: Personalized financial advice                           │
│ ADVISOR │  FR-5.4: Spending pattern analysis                               │
│         │  FR-5.5: Budget recommendations                                  │
│         │  FR-5.6: Anomaly detection in spending                           │
│         │  FR-5.7: Financial goal planning assistance                      │
│         │  FR-5.8: Tool calling for real-time data access                  │
├─────────┼──────────────────────────────────────────────────────────────────┤
│         │  FR-6.1: Create financial goals                                  │
│  GOALS  │  FR-6.2: Track goal progress                                     │
│         │  FR-6.3: AI-suggested savings plans                              │
│         │  FR-6.4: Goal completion notifications                           │
└─────────┴──────────────────────────────────────────────────────────────────┘
```
...
│  │  GSI-1: email-index                                                 │    │
│  │  ├─ PK: email                                                       │    │
│  │  └─ Projects: userId, passwordHash, isActive                        │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  TABLE: Transactions                                                         │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  Partition Key (PK)  │  USER#<userId>                               │    │
│  │  Sort Key (SK)       │  TXN#<timestamp>#<transactionId>             │    │
│  ├─────────────────────────────────────────────────────────────────────┤    │
│  │  Attributes:                                                        │    │
...
│  │  └─ tags             : List<String>                                 │    │
│  ├─────────────────────────────────────────────────────────────────────┤    │
│  │  GSI-1: category-date-index                                         │    │
│  │  ├─ PK: USER#<userId>#CAT#<category>                                │    │
│  │  ├─ SK: transactionDate                                             │    │
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  Partition Key (PK)  │  USER#<userId>                               │    │
│  │  Sort Key (SK)       │  BUDGET#<YYYY-MM>#<category>                 │    │
│  ├─────────────────────────────────────────────────────────────────────┤    │
│  │  Attributes:                                                        │    │
...
│  │  ├─ spentAmount      : Number                                       │    │
│  │  ├─ currency         : String                                       │    │
│  │  ├─ alertThresholds  : List<Number> [50, 75, 90, 100]               │    │
│  │  ├─ alertsSent       : List<Number>                                 │    │
│  │  ├─ createdAt        : String                                       │    │
│  │  └─ updatedAt        : String                                       │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
...
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │  Partition Key (PK)  │  USER#<userId>                               │    │
│  │  Sort Key (SK)       │  CONV#<conversationId>#MSG#<timestamp>       │    │
│  ├─────────────────────────────────────────────────────────────────────┤    │
│  │  Attributes:                                                        │    │
...
│    ▼        ▼                        ▼                     ▼                │
│ ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────┐           │
│ │   TRANSACTION    │  │     BUDGET       │  │ AI_CONVERSATION  │           │
│ ├──────────────────┤  ├──────────────────┤  ├──────────────────┤           │
│ │ PK: transactionId│  │ PK: budgetId     │  │ PK: conversationId│          │
│ │ FK: userId       │  │ FK: userId       │  │ FK: userId       │           │
│ │ type             │  │ category         │  │ messages[]       │           │
│ │ amount           │  │ monthYear        │  │ createdAt        │           │
│ │ category         │  │ limitAmount      │  │ lastMessageAt    │           │
│ │ subcategory      │  │ spentAmount      │  │ tokenCount       │           │
│ │ description      │  │ alertThresholds[]│  └──────────────────┘           │
│ │ transactionDate  │  │ alertsSent[]     │                                 │
│ │ isRecurring      │  └──────────────────┘           │                     │
│ │ tags[]           │                                 │                     │
│ └──────────────────┘                                 │                     │
│         │                                            │                     │
...
│         ▼                                            ▼                     │
│ ┌──────────────────┐                     ┌──────────────────┐              │
│ │  BUDGET (spent)  │                     │  FINANCIAL_GOAL  │              │
│ │  Auto-updated    │                     ├──────────────────┤              │
│ │  on transaction  │                     │ PK: goalId       │              │
│ │  create/update   │                     │ FK: userId       │              │
│ └──────────────────┘                     │ name             │              │
│                                          │ targetAmount     │              │
│                                          │ currentAmount    │              │
│                                          │ targetDate       │              │
...
│  └─ GET    /budgets/status       - Get budget status summary               │
│                                                                              │
│  📊 ANALYTICS (/analytics)                                                  │
│  ├─ GET    /analytics/summary    - Get financial summary                   │
│  ├─ GET    /analytics/spending   - Get spending breakdown                  │
│  ├─ GET    /analytics/trends     - Get spending trends                     │
│  ├─ GET    /analytics/categories - Get category-wise analysis              │
...
│  │   ├─ Caffeine cache for hot data                                    │    │
│  │   ├─ TTL: 5 minutes                                                 │    │
│  │   ├─ Size: 1000 entries                                             │    │
│  │   └─ Use: Frequently accessed, read-heavy data                      │    │
│  │                                                                      │    │
│  │   L2 CACHE (Redis)                                                  │    │
│  │   ├─ Distributed cache                                              │    │
...
│  │   Monthly Transactions     │  15 minutes   │  Moderate updates       │    │
│  │   Budget Status            │  5 minutes    │  Frequent updates       │    │
│  │   Analytics Summary        │  30 minutes   │  Computed data          │    │
│  │   AI Conversation Context  │  2 hours      │  Session-based          │    │
...
│                                                                               │
│   ┌────────┐    ┌────────────┐    ┌────────────┐    ┌──────────┐            │
│   │ Client │    │ Controller │    │  Service   │    │ DynamoDB │            │
│   └───┬────┘    └─────┬──────┘    └─────┬──────┘    └────┬─────┘            │
│       │               │                 │                 │                  │
│       │ POST /register│                 │                 │                  │
│       │──────────────►│                 │                 │                  │
...
│                                                                               │
│  ┌────────┐  ┌──────┐  ┌────────────┐  ┌─────────┐  ┌──────────┐  ┌───────┐ │
│  │ Client │  │ JWT  │  │ Controller │  │ Service │  │ DynamoDB │  │ Redis │ │
│  └───┬────┘  └──┬───┘  └─────┬──────┘  └────┬────┘  └────┬─────┘  └───┬───┘ │
│      │          │            │              │            │            │      │
│      │ POST /transactions    │              │            │            │      │
│      │ + JWT Token          │              │            │            │      │
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│                    AI CHAT FLOW WITH FUNCTION CALLING                         │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                               │
...
│   │ └─ Gemini decides to call: getTransactionsByCategory("Food")        │    │
│   └─────────────────────────────────────────────────────────────────────┘    │
│                              │                                                │
│                              ▼                                                │
│   ┌─────────────────────────────────────────────────────────────────────┐    │
│   │ STEP 4: Execute Function Call                                       │    │
│   │ ├─ FinancialTools.getTransactionsByCategory("Food", "2026-01-01",  │    │
│   │ │                                            "2026-01-31")          │    │
│   │ ├─ Query DynamoDB via TransactionService                            │    │
│   │ └─ Return: [{amount: 45.99, desc: "Restaurant"}, ...]               │    │
│   └─────────────────────────────────────────────────────────────────────┘    │
│                              │                                                │
│                              ▼                                                │
│   ┌─────────────────────────────────────────────────────────────────────┐    │
│   │ STEP 5: Generate Final Response                                     │    │
│   │ ├─ Gemini processes function results                                │    │
...
│   │ ├─ Update context cache in Redis                                    │    │
│   │ └─ Return response to user                                          │    │
│   └─────────────────────────────────────────────────────────────────────┘    │
│                                                                               │
│   Response: "This month you've spent $342.50 on Food & Dining..."           │
│                                                                               │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
│  ├─ DynamoDB local setup and table creation                                 │
│  ├─ Redis Docker setup                                                      │
│  ├─ Basic entity models                                                     │
...
│                                                                              │
│  PHASE 7: Frontend (Week 10-11)                                             │
│  ├─ Basic UI with Thymeleaf/React                                           │
│  ├─ Dashboard                                                               │
│  ├─ Transaction forms                                                       │
│  ├─ Chat interface                                                          │
...
│                ┌─────────────────────────────────┐                          │
│               /│         Unit Tests              │\                         │
│              / │    (Services, Utils, etc.)      │ \     ~80% of tests     │
│             /  │                                 │  \    Fastest, stable    │
│            /   └─────────────────────────────────┘   \                      │
│                                                                              │
